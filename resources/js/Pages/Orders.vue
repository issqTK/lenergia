                ...
                <div class="p-6 text-gray-900">
                    <div class="overflow-x-scroll overscroll-contain scroll-y">
                        <table class="orders rounded overflow-hidden" v-if="this.current == 'list'">
                            ...
                            <tr v-for="da in data" :key="da.id" @click="show(da)"
                                class="relative cursor-pointer overflow-hidden">

                                <td>{{ da.reviewState }}</td>

                                <td>{{ da.client.gender == 'Monsieur' ? 'Mr' : 'Mme' }} {{ da.client.lastName }} {{
                                    da.client.firstName }}</td>
                                ...
                                <td class="leading-3">{{ da.logementEnergie }} <br>
                                    <span
                                        v-if="da.logementEnergie == 'Chauffage au fioul' || da.logementEnergie == 'Chauffage électrique' || da.logementEnergie == 'Chauffage au gaz'">-
                    ...
                    <OrderView v-if="current == 'view'" :view=this.view @current=this.getCurrent></OrderView>
                </div>
            </div>
        </div>
    </div>
