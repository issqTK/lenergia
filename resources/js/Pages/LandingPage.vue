        ...
        <div class="flex mt-4 gap-6">
                ...
                <div class="w-3/4 m-auto">
                    ...
                    <ul class="flex flex-col gap-4 break-words">
                        <li class="flex gap-2" v-for="article in articles" :key="article.id">
                            ...
                            <a @click.prevent="toArticle(article.link_id)" class="shadow-text cursor-pointer">{{article.title}}</a>
                        </li>
                        <li class="flex gap-2">
                            <i class="fa-solid leading-6 fa-circle" style="color: #72efa6; font-size:.5em" ></i>
            ...
            <div class="text-slate-800 w-full md:w-8/12 px-4 md:px-12 py-6 flex flex-col gap-8 bg-white rounded" >
                ...
                <div class="md:hidden flex flex-col gap-2">
                    ...
                    <ul class="flex flex-col gap-4 break-words p-0 rounded bg-white text-sm h-0 overflow-hidden bg-slate-100">
                        <li class="flex gap-2" v-for="article in articles" :key="article.id">
                            <i class="fa-solid leading-6 fa-circle" style="color: #72efa6; font-size:.5em" ></i>
                            <a @click.prevent="toArticle(article.link_id)" class="hover:font-semibold cursor-pointer">{{article.title}}</a>
                        </li>
                    </ul>
                </div>
                ...
                <div class="flex flex-col gap-6">
                    ...
                    <!-- description -->
                    <p v-html="work.description" class="break-words leading-8" style="word-spacing: 1px; letter-spacing: .5px;"></p>

                    <!--  -->
                    <p style="word-spacing: .7px; letter-spacing: .2px; text-align: center;">
                        <strong v-if="work.id == 1" class="text-center">Optez pour l'offre Panneaux Solaires Photovoltaïques et commencez à produire votre propre électricité !</strong>
                        <strong v-if="work.id == 2" class="text-center">Profitez d'une simulation gratuite pour estimer votre devis solaire et découvrir le montant potentiel de vos économies.</strong>
                        <strong v-if="work.id == 3" class="text-center">Profitez de l'utilisation de panneaux hybrides pour réaliser jusqu'à 80 % d'économies sur vos coûts énergétiques. 👇</strong>
                        <strong v-if="work.id == 4" class="text-center">Optez pour un choix gagnant avec le cumulus thermodynamique pour des économies d'énergie et une empreinte carbone réduite.</strong>
                        <strong v-if="work.id == 5" class="text-center">Envie d'installer une pompe à chaleur ? Obtenez votre devis gratuit dès maintenant !</strong>
                        <strong v-if="work.id == 6" class="text-center">Passez au poêle à granulés ! Obtenez votre devis gratuit aujourd'hui</strong>
                        <strong v-if="work.id == 7" class="text-center">Dans cet article, nous allons explorer l'essentiel du Système Solaire Combiné, son fonctionnement, ainsi que les coûts associés et les subventions disponibles.</strong>
                        <strong v-if="work.id == 8" class="text-center">Faites le calcule de vos aides pour l'isolation intérieure de vos murs !</strong>
                        <strong v-if="work.id == 9" class="text-center">Calculez vos aides pour l'isolation des murs par l'extérieur !</strong>
                    </p>
                </div>
                ...
                <div class="article flex flex-col gap-8" v-for="article in articles" :key="article.id">
                    <h2 class="text-xl md:text-2xl font-bold" :ref="article.link_id">{{ article.title }}</h2>
                ...
                <div id="faq" class="flex flex-col gap-4">
                    ...
                    <div v-for="faq in faqs" :key="faqs.id" class="flex flex-col gap-3 p-3 bg-slate-200 text-black rounded hover:bg-slate-100">
...
<style>
.shadow-text:hover{text-shadow: .5px 0px 1px;}

.little-article img { width: 90%; margin: 0 auto; }

#faq p, .little-article p, .little-article i, .little-article ul { word-break: break-word; font-size: .98em; line-height: 26px; word-spacing: .8px; letter-spacing: .5px;}
...
.little-article table tr:first-child { background-color: #64748b; color: white; font-weight: bold; font-size: .75em!important; }

.little-article table tr:nth-child(even) { background-color: #f8fafc; }

.little-article table td {padding: 15px; text-align: center; font-size: .8em;}

.little-article ul{ list-style-type:disc; list-style-position: outside; margin-left:15px; }
